    ...
    <BreadcrumbWrapper>
      {
        breadcrumbs.map(({ match, breadcrumb }) => {
          ...
          if (id === Object.keys(dataNameById)[0]) {
            return <Link className='breadcrumb-item' to={match.pathname} key=
              {match.pathname}>{dataNameById[id]}</Link>
          }

          return <Link className='breadcrumb-item' to={match.pathname} key={match.pathname}>
            {breadcrumb}
          </Link>
