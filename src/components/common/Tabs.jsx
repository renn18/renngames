    ...
    <TabsWrapper className='bg-white'>
      <div className='container'>
        <div className='tabs-content'>
          <ul className={`tabs-button${tabButtonStatus ? "-show" : ""}`}>
            <button type="button" className='tabs-buttons-close bg-white d-flex 
            align-items-center justify-content-center' onClick={tabButtonsHandler}>
                ...
                return (
                  <li key={item?.id} className={`tabs-button ${item?.id === activeTab.id ? 'tabs-active' : ""}`}>
                    <button className='text-white' type="button" onClick={() =>
                      tabClickHandler(item?.id)}>{item?.name}</button>
                  </li>
                )
              })
            }
